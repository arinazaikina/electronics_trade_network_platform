...
class CustomUserAdmin(UserAdmin):
    ...
    def has_module_permission(self, request: HttpRequest) -> bool:
        """
        Определяет, имеет ли пользователь разрешение на модуль.

        :param request: Запрос HTTP от пользователя.
        :return: Возвращает True, если пользователь является суперпользователем.

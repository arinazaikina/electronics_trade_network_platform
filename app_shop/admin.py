...
class SupplierAdmin(BaseAdmin):
    ...
    - В списке отображаются поля: ID, тип поставщика, имя, ссылка на родительского поставщика,
      количество посредников и задолженность.
    - Для фильтрации доступна опция по городу.
    - Предоставляет возможность массового обнуления задолженности у выбранных поставщиков.
    - Производится автоматическая проверка наличия задолженности перед удалением.
    - При создании или изменении поставщика проверяется наличие циклических зависимостей.

    Основные методы:
    - delete_model и delete_queryset: удаляют поставщика или группу поставщиков,
      учитывая наличие задолженности и переназначая детей на родительский уровень.
    - save_model: сохраняет модель поставщика, проверяя наличие циклических зависимостей.
    ...
    @staticmethod
    def _reassign_children_to_parent(obj: Supplier) -> None:
        """
        Переназначает детей поставщика на его родительское звено.

        :param obj: Поставщик, для которого требуется переназначение детей.
        """
    ...
    @staticmethod
    def _show_debt_error_message(request: HttpRequest, obj: Supplier, debtor: Supplier) -> None:
        """
        Показывает сообщение об ошибке при попытке удалить поставщика с задолженностью.

        :param request: Запрос Django.
        :param obj: Поставщик, для которого проверяется задолженность.
        :param debtor: Поставщик с задолженностью.
        """
        if debtor == obj:
    ...
    def _handle_deletion(self, request: HttpRequest, obj: Supplier) -> bool:
        """
        Обрабатывает удаление поставщика, учитывая наличие задолженности.

        :param request: Запрос Django.
        :param obj: Поставщик, который подлежит удалению.
    ...
    def delete_queryset(self, request: HttpRequest, queryset: QuerySet) -> None:
        ...
        :param request: Запрос Django.
        :param queryset: QuerySet поставщиков, который подлежит удалению.
        """
    ...
    def save_model(self, request: HttpRequest, obj: Supplier, form: ModelForm, change: bool) -> None:
        ...
        :param request: Запрос Django.
        :param obj: Поставщик, который сохраняется.
        :param form: Форма для модели.
        :param change: Флаг, указывающий на изменение существующего объекта.
        """
        try:
    ...
    def response_action(self, request: HttpRequest, queryset: QuerySet) -> HttpResponse:
        """
        Обрабатывает ответ после выполнения какого-либо действия над набором объектов.

        :param request: Запрос Django.
        :param queryset: QuerySet объектов, над которыми было выполнено действие.
        :return: HttpResponse с результатом обработки.
        """
    ...
    def link_to_parent(self, obj: Supplier) -> str:
        """
        Возвращает ссылку на родительский объект поставщика.

        :param obj: Поставщик, для которого необходимо получить ссылку.
        :return: Строка, представляющая собой HTML-ссылку на родительский объект.
        """
        if obj.parent:
    ...
    def number_of_intermediaries(self, obj: Supplier) -> Union[int, str]:
        """
        Возвращает количество посредников для данного поставщика.

        :param obj: Поставщик, для которого необходимо определить количество посредников.
        :return: Число посредников или строковое представление.
        """
        level = obj.get_level()
    ...
    def clear_debt(self, request: HttpRequest, queryset: QuerySet) -> None:
        ...
        :param request: Запрос Django.
        :param queryset: Список поставщиков, для которых необходимо очистить задолженность.
        """
